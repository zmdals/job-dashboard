...
public class CareerController {
    ...
    @Operation(summary = "전체 경력 조회", description = "등록된 모든 경력사항을 조회합니다")

    @GetMapping
    public ApiResponse<List<CareerRes>> getAll() {
        return ApiResponse.ok(careerService.getAll());
    }

    @Operation(summary = "경력 상세 조회", description = "careerId로 경력사항을 조회합니다")
    @GetMapping("/{careerId}")
    public ApiResponse<CareerRes> get(@PathVariable Long careerId) {
        return ApiResponse.ok(careerService.get(careerId));
    }

...
const json = (body, status = 200) => HttpResponse.json(body, { status })
...
function findCompany(db, companyId) {
  const normalizedCompanyId = /^\d+$/.test(String(companyId))
    ? `company-${String(companyId).padStart(3, '0')}`
    : companyId
  const company = (db.companies ?? []).find((item) => item.id === normalizedCompanyId)
  ...
  const legacyPosting = db.postings.find(
    (posting) =>
      ...
      posting.companyId === normalizedCompanyId ||
      posting.company?.id === normalizedCompanyId,
  )
...
export const handlers = [
  http.post(`${API}/auth/login`, async ({ request }) => {
    ...
    const user = db.users.find(
      (candidate) => candidate.email === id && candidate.password === password,
    )

    if (!user) {
      return error(401, 'INVALID_CREDENTIALS', '아이디 또는 비밀번호가 올바르지 않습니다.')
    }
  ...
  http.post(`${API}/auth/signup`, async ({ request }) => {
    ...
    if (!id || !password || !email || !name) {
      return error(400, 'INVALID_REQUEST', 'id, password, email, name은 필수입니다.')
    }

    if (db.users.some((user) => user.loginId === id)) {
      return error(409, 'DUPLICATE_LOGIN_ID', '이미 사용 중인 아이디입니다.')
    }

    if (db.users.some((user) => user.email === email)) {
      return error(409, 'DUPLICATE_EMAIL', '이미 사용 중인 이메일입니다.')
    }
  ...
  http.get(`${API}/companies/:companyId`, async ({ params }) => {
    ...
    const company = findCompany(getDb(), params.companyId)
  ...
  http.get(`${API}/companies/:companyId/evidences`, async ({ params }) => {
    ...
    const db = getDb()
    const company = findCompany(db, params.companyId)

    if (!company) {
      return error(404, 'COMPANY_NOT_FOUND', '회사를 찾을 수 없습니다.')
    }

    return json(db.companyEvidencesById?.[company.id] ?? [])
  }),
  ...
  http.get(`${API}/postings/:postingId`, async ({ params }) => {
    ...
    if (!posting) {
      return error(404, 'POSTING_NOT_FOUND', '공고를 찾을 수 없습니다.')
    }
  ...
  http.post(`${API}/postings`, async ({ request }) => {
    ...
    if (!body?.companyId || !body?.title) {
      return error(400, 'INVALID_REQUEST', 'companyId와 title은 필수입니다.')
    }
  ...
  http.put(`${API}/postings/:postingId`, async ({ request, params }) => {
    ...
    const db = getDb()
    const index = db.postings.findIndex((posting) => posting.id === params.postingId)
  ...
  http.delete(`${API}/postings/:postingId`, async ({ params }) => {
    ...
    db.postings = db.postings.filter((posting) => posting.id !== params.postingId)
  ...
  http.get(`${API}/postings/:postingId/info`, async ({ params }) => {
    ...
    if (!info) {
      return error(404, 'POSTING_INFO_NOT_FOUND', '공고 관련 정보를 찾을 수 없습니다.')
    }
  ...
  http.post(`${API}/postings/:postingId/applications`, async ({ request, params }) => {
    await latency()
    ...
    const alreadyApplied = db.applications.some(
      (application) => application.userId === user.id && application.postingId === posting.id,
    )

    if (alreadyApplied) {
      return error(409, 'ALREADY_APPLIED', '이미 지원한 공고입니다.')
    }
  ...
  http.post(`${API}/me/starred-postings/:postingId`, async ({ request, params }) => {
    ...
    if (starredIds.includes(params.postingId)) {
      return error(409, 'ALREADY_STARRED', '이미 즐겨찾기한 공고입니다.')
    }
  ...
  http.delete(`${API}/me/starred-postings/:postingId`, async ({ request, params }) => {
    ...
    db.starredByUserId[user.id] = (db.starredByUserId[user.id] ?? []).filter(
      (postingId) => postingId !== params.postingId,
    )
  ...
  http.put(`${API}/me/educations/:educationId`, async ({ request, params }) => {
    await latency()
    ...
    const index = (user.educations ?? []).findIndex((item) => item.id === params.educationId)
  ...
  http.delete(`${API}/me/educations/:educationId`, async ({ request, params }) => {
    await latency()
    ...
    user.educations = (user.educations ?? []).filter((item) => item.id !== params.educationId)
  ...
  http.put(`${API}/me/careers/:careerId`, async ({ request, params }) => {
    await latency()
    ...
    const index = (user.careers ?? []).findIndex((item) => item.id === params.careerId)
  ...
  http.delete(`${API}/me/careers/:careerId`, async ({ request, params }) => {
    await latency()
    ...
    user.careers = (user.careers ?? []).filter((item) => item.id !== params.careerId)
  ...
  http.put(`${API}/me/projects/:projectId`, async ({ request, params }) => {
    await latency()
    ...
    const index = (user.projects ?? []).findIndex((item) => item.id === params.projectId)
  ...
  http.delete(`${API}/me/projects/:projectId`, async ({ request, params }) => {
    await latency()
    ...
    user.projects = (user.projects ?? []).filter((item) => item.id !== params.projectId)
  ...
  http.post(`${API}/me/certificates`, async ({ request }) => {
    ...
    if (!body?.certName) {
      return error(400, 'INVALID_REQUEST', '자격증 이름은 필수입니다.')
    }
  ...
  http.put(`${API}/me/certificates/:certificateId`, async ({ request, params }) => {
    await latency()
    ...
    const index = (user.certificates ?? []).findIndex((item) => item.id === params.certificateId)
  ...
  http.delete(`${API}/me/certificates/:certificateId`, async ({ request, params }) => {
    await latency()
  ...
  http.patch(`${API}/applications/:applicationId/status`, async ({ request, params }) => {
    ...
    const application = db.applications.find((item) => item.id === params.applicationId)

    if (!application) {
      return error(404, 'APPLICATION_NOT_FOUND', '지원 정보를 찾을 수 없습니다.')
    }
  ...
  http.delete(`${API}/applications/:applicationId`, async ({ request, params }) => {
    await latency()
